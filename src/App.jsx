...
import {
  ...
  Calculator, CalendarDays, Bell, Download, Upload,
} from "lucide-react";
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!parsed || !parsed.data || !Array.isArray(parsed.data.members)) {
...
function LoginScreen({ onEnter }) {
  ...
  async function handleCreate() {
    if (!name.trim() || !householdName.trim()) { setError("Preencha seu nome e o nome do ambiente familiar."); return; }
    setBusy(true);
    const newCode = Math.random().toString(36).slice(2, 8).toUpperCase();
    const data = {
      name: householdName.trim(), members: [{ name: name.trim() }],
      incomes: [], expenses: [], goals: [],
      emergencyFund: { avgMonthlyExpense: 0, monthsDesired: 6, currentSaved: 0, goalMonths: 12, target: 0 },
      createdAt: new Date().toISOString(),
    };
    await saveHousehold(newCode, data);
    await saveSession({ code: newCode, name: name.trim() });
    setBusy(false);
    onEnter(newCode, name.trim(), data);
  }

  async function handleJoin() {
    if (!name.trim() || !code.trim()) { setError("Preencha seu nome e o código do ambiente familiar."); return; }
    setBusy(true);
    const cleanCode = code.trim().toUpperCase();
    const data = await loadHousehold(cleanCode);
    if (!data) { setError("Código não encontrado. Confira com seu parceiro(a)."); setBusy(false); return; }
    if (!data.members.find((m) => m.name.toLowerCase() === name.trim().toLowerCase())) {
      data.members.push({ name: name.trim() });
      await saveHousehold(cleanCode, data);
    }
    await saveSession({ code: cleanCode, name: name.trim() });
    setBusy(false);
    onEnter(cleanCode, name.trim(), data);
  }
  ...
  async function handleRestore() {
    ...
    setBusy(true);
    const restoredCode = backupParsed.code || Math.random().toString(36).slice(2, 8).toUpperCase();
    const data = backupParsed.data;
    if (!data.members.find((m) => m.name.toLowerCase() === name.trim().toLowerCase())) {
      data.members.push({ name: name.trim() });
    }
    await saveHousehold(restoredCode, data);
    await saveSession({ code: restoredCode, name: name.trim() });
    setBusy(false);
    onEnter(restoredCode, name.trim(), data);
  }

  const submit = mode === "create" ? handleCreate : mode === "join" ? handleJoin : handleRestore;
  const submitLabel = mode === "create" ? "Criar e continuar" : mode === "join" ? "Entrar no ambiente" : "Restaurar e continuar";
    ...
    <div className="min-h-screen flex items-center justify-center bg-stone-50 px-4">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          <DuoRings size={52} />
          <h1 style={{ fontFamily: "Fraunces" }} className="text-2xl font-semibold text-stone-800 mt-3 text-center">Controle e Gestão Financeira</h1>
          <p className="text-sm text-stone-500 mt-1 text-center">O consultor financeiro do casal, sempre disponível.</p>
        </div>
        {!hasRealBackend && (
          <div className="flex items-start gap-2 bg-amber-50 text-amber-700 text-xs rounded-lg px-3 py-2.5 mb-3">
            <AlertTriangle size={15} className="shrink-0 mt-0.5" />
            <span>Banco de dados ainda não configurado — os dados ficam salvos só neste navegador e <b>não sincronizam</b> com seu parceiro(a) ainda. Veja o README para conectar o Supabase.</span>
          </div>
        )}
        <Card className="p-5">
          <div className="flex bg-stone-100 rounded-lg p-1 mb-5">
            <button onClick={() => { setMode("create"); setError(""); }}
              className={`flex-1 text-xs sm:text-sm font-medium py-1.5 rounded-md transition-colors ${mode === "create" ? "bg-white shadow-sm text-teal-900" : "text-stone-500"}`}>Criar</button>
            <button onClick={() => { setMode("join"); setError(""); }}
              className={`flex-1 text-xs sm:text-sm font-medium py-1.5 rounded-md transition-colors ${mode === "join" ? "bg-white shadow-sm text-teal-900" : "text-stone-500"}`}>Entrar com código</button>
            <button onClick={() => { setMode("restore"); setError(""); }}
              className={`flex-1 text-xs sm:text-sm font-medium py-1.5 rounded-md transition-colors ${mode === "restore" ? "bg-white shadow-sm text-teal-900" : "text-stone-500"}`}>Restaurar backup</button>
          </div>
          <div className="space-y-3">
            <Field label="Seu nome">
              <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: João" />
            </Field>
            {mode === "create" && (
              <Field label="Nome do ambiente financeiro">
                <input className={inputCls} value={householdName} onChange={(e) => setHouseholdName(e.target.value)} placeholder="Ex: João & Maria" />
              </Field>
            )}
            {mode === "join" && (
              <Field label="Código do ambiente familiar">
                <input className={inputCls} value={code} onChange={(e) => setCode(e.target.value)} placeholder="Ex: 7F3K2A" />
              </Field>
            )}
            {mode === "restore" && (
              <Field label="Arquivo de backup (.json)">
                ...
                {backupFileName && <p className="text-xs text-emerald-600 mt-1.5">Arquivo carregado: {backupFileName}</p>}
              </Field>
            )}
            {error && <p className="text-xs text-rose-600">{error}</p>}
            <button disabled={busy} onClick={submit}
              className="w-full bg-teal-900 hover:bg-teal-800 text-white text-sm font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-60">
              {busy ? "Aguarde..." : submitLabel} <ArrowRight size={16} />
            </button>
          </div>
        </Card>
        <p className="text-xs text-stone-400 text-center mt-4">
          {mode === "create" && "Depois de criar, compartilhe o código gerado com seu parceiro(a) em Ajustes."}
          {mode === "join" && "Peça o código de 6 caracteres para quem já criou o ambiente."}
          {mode === "restore" && "Use o arquivo baixado em Ajustes → Backup manual, num aparelho anterior."}
        </p>
      </div>
    </div>
  );
...
/* ============================== CONFIGURAÇÕES ============================== */
function ConfigScreen({ data, code, session, mutate, onLogout }) {
  ...
  return (
    <div className="space-y-5">
      <h1 style={{ fontFamily: "Fraunces" }} className="text-2xl font-semibold text-stone-800">Ajustes</h1>
      ...
      <Card className="p-4">
        ...
        <div className="flex gap-2 mb-3">
          ...
          <button onClick={saveName} className="bg-teal-900 hover:bg-teal-800 text-white text-sm font-medium px-4 rounded-lg shrink-0">Salvar</button>
        </div>
      ...
      <Card className="p-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={handleExport} className="flex items-center gap-1.5 bg-stone-100 hover:bg-stone-200 text-stone-700 text-sm font-medium px-4 py-2 rounded-lg"><Download size={15} />Baixar backup</button>
          <label className="flex items-center gap-1.5 bg-stone-100 hover:bg-stone-200 text-stone-700 text-sm font-medium px-4 py-2 rounded-lg cursor-pointer">
            ...
            <input type="file" accept="application/json,.json" className="hidden" onChange={handleImportChange} />
          </label>
        </div>
        {importMsg && <p className={`text-xs mt-2 ${importMsg.ok ? "text-emerald-600" : "text-rose-600"}`}>{importMsg.text}</p>}
      </Card>
      ...
      <button onClick={onLogout} className="flex items-center gap-2 text-sm font-medium text-rose-600 hover:text-rose-700 px-1">
...
export default function App() {
  ...
  function handleEnter(code, name, initialData) {
  ...
  if (session === undefined) {
    return <div className="min-h-screen flex items-center justify-center bg-stone-50"><DuoRings size={44} /></div>;
  }
  if (!session || !data) {
    return (
      <>
  ...
  const screens = {
    ...
    config: <ConfigScreen data={data} code={session.code} session={session} mutate={mutate} onLogout={handleLogout} />,
  };
